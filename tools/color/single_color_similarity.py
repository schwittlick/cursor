...
class ColorComparisonApp(QWidget):
    ...
    def compare_colors(self):
        ...
        sorted_colors = sorted(
            self.copic.available_colors.values(),
            key=lambda c: colour.delta_E(
                target_lab, colour.XYZ_to_Lab(colour.sRGB_to_XYZ(c.as_srgb())), method="CIE2000"
            ),
        )
        ...
        for i, color in enumerate(sorted_colors[:5], 1):
            delta = colour.delta_E(target_lab, colour.XYZ_to_Lab(colour.sRGB_to_XYZ(color.as_srgb())), method="CIE2000")
