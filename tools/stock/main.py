...
def realtime_update_plot():
    ...
    close = float(price_data['last_close'])
    close_str = "{:.10f}".format(close)
    print(close_str)
...
def create_ctrl_panel(win):
    ...
    [panel.symbol.addItem(i+'USDT') for i in 'PEPE BTC ETH XTZ'.split()]
    ...
    panel.interval.setCurrentIndex(2)
    ...
    panel.indicators.setCurrentIndex(2)
...
ax,ax_rsi = fplt.create_plot('Complicated Binance Example', rows=2, init_zoom_periods=50)
axo = ax.overlay()
