...
class Sorter:
    ...
    def sort(
        ...
        if self.__param is SortParameter.ENTROPY_X:
            ...
            paths.sort(key=lambda x: x.entropy_direction_changes, reverse=self.__reverse)
        ...
        elif self.__param is SortParameter.FRECHET_DISTANCE and reference_path is not None:
            ...
            raise Exception(f"Unknown parameter {self.__param} for {__class__.__name__}")
    ...
    def sorted(
        ...
        if self.__param is SortParameter.ENTROPY_X:
            sorted_list = sorted(paths, key=lambda x: x.entropy_x, reverse=self.__reverse)
        elif self.__param is SortParameter.ENTROPY_Y:
            sorted_list = sorted(paths, key=lambda x: x.entropy_y, reverse=self.__reverse)
        elif self.__param is SortParameter.ENTROPY_CROSS:
            sorted_list = sorted(paths, key=lambda x: x.entropy_y * x.entropy_x, reverse=self.__reverse)
        elif self.__param is SortParameter.DIFFERENTIAL_ENTROPY_X:
            sorted_list = sorted(paths, key=lambda x: x.differential_entropy_x, reverse=self.__reverse)
        elif self.__param is SortParameter.DIFFERENTIAL_ENTROPY_Y:
            sorted_list = sorted(paths, key=lambda x: x.differential_entropy_y, reverse=self.__reverse)
        elif self.__param is SortParameter.DIFFERENTIAL_ENTROPY_CROSS:
            sorted_list = sorted(
                paths,
                key=lambda x: x.differential_entropy_x * x.differential_entropy_y,
            ...
            sorted_list = sorted(paths, key=lambda x: x.distance, reverse=self.__reverse)
        elif self.__param is SortParameter.HASH:
            ...
            if use_multiprocessing:
                ...
                distances = [(index, item.frechet_similarity(reference_path), item) for index, item in enumerate(paths)]

            sorted_idxes = sorted(distances, key=itemgetter(1), reverse=self.__reverse)
            sorted_list = [el[2] for el in sorted_idxes]
        elif self.__param is SortParameter.VARIATION_X:
            sorted_list = sorted(paths, key=lambda x: x.variation_x, reverse=self.__reverse)
        elif self.__param is SortParameter.VARIATION_Y:
            sorted_list = sorted(paths, key=lambda x: x.variation_y, reverse=self.__reverse)
        else:
