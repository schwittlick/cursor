...
from cursor.device import MinmaxMapping, PlotterHpglNames
...
def create_file_dialogue(cb: typing.Callable):
    with dpg.file_dialog(directory_selector=False, show=False, callback=cb, tag="file_dialog", width=700, height=400):
...
def create_plotter_inspector_gui(inspector: SerialInspector):
    with dpg.window(label="Inspector"):
        ...
        with dpg.group(horizontal=True):
            dpg.add_button(
                label="Random Swirl (ink drink)", callback=lambda: inspector.send_command(generate_random_swirl())
            )
            dpg.add_button(label="Random dot walk", callback=lambda: inspector.send_command(generate_random_dot_walk()))
            dpg.add_button(
                label="Fill square (dotted)",
                callback=lambda: inspector.send_command(generate_filled_square_dotted(inspector)),
            )
            dpg.add_button(
                label="Fill square (lines)",
...
def create_bruteforce_gui(inspector: SerialInspector):
    with dpg.window(label="Bruteforce", pos=(0, 600)):
        ...
        with dpg.group(horizontal=True):
            dpg.add_button(
                label="Bruteforce", tag="start_bruteforce_button", callback=inspector.start_bruteforce_progress
            )
            dpg.add_button(
                label="Stop Bruteforce", tag="stop_bruteforce_button", callback=inspector.stop_bruteforce_progress
            )
            dpg.add_combo(
                label="Timeout",
                items=["0.1", "0.3", "0.7", "1.0", "2.0"],
                default_value="1.0",
                tag="timeout_dropdown",
                width=100,
            )
