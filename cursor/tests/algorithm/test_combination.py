from cursor.algorithm.color.combinations import ColorDictionary, Color as CombinationColor
...
def test_more():
    ...
    print(
        f"Most similar Copic color: {similar_copic.name} ({similar_copic.code.name}) (#{similar_copic.as_rgb()[0]:02x}{similar_copic.as_rgb()[1]:02x}{similar_copic.as_rgb()[2]:02x})")
    ...
    for i, color in enumerate(sorted_colors[:5], 1):
        ...
        print(
            f"{i}. {color.name} ({color.code.name}) (#{color.as_rgb()[0]:02x}{color.as_rgb()[1]:02x}{color.as_rgb()[2]:02x}) - Delta E: {delta:.2f}")
